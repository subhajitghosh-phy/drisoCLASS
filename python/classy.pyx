    ...
    def compute(self, level=["lensing"]):
        ...
        if "input" in level:
            if input_init(&self.fc, &self.pr, &self.ba, &self.th,
                ...
                raise CosmoComputationError(errmsg)#CosmoSevereError(errmsg)##SG
            self.ncp.add("input")
...
#SG
                ...
                value = self.pm.n_ad_cdi
            elif name == 'f_nid':
                value = self.pm.f_nid
            elif name == 'n_nid':
                value = self.pm.n_nid
            elif name == 'c_ad_nid':
                value = self.pm.c_ad_nid
            elif name == 'n_ad_nid':
                value = self.pm.n_ad_nid
            elif name == 'f_drid':
                value = self.pm.f_drid
            elif name == 'n_drid':
                value = self.pm.n_drid
            elif name == 'c_ad_drid':
                value = self.pm.c_ad_drid
            elif name == 'n_ad_drid':
                value = self.pm.n_ad_drid
            elif name == 'frac_idr':#frac_idr = N_idr/(N_idr+N_ur+N_ncdm)
                value = self.ba.Omega0_idr/(self.ba.Omega0_idr + self.ba.Omega0_ur + self.ba.Omega0_ncdm_tot)
