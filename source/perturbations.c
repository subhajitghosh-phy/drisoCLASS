    ...
    */
    ...
    if ((ppt->has_ad == _TRUE_) && (index_ic == ppt->index_ic_ad)) {
      ...
      ppw->pv->y[ppw->pv->index_pt_theta_g] = - k*ktau_three/36. * (1.-3.*(1.-fracnu-fracdr)/20./(1.-fracnu-fracdr)*om*tau)  * ppr->curvature_ini * s2_squared; 
       ...
       ppw->pv->y[ppw->pv->index_pt_theta_g] = -k*ktau_three/36. * (1.-3.*(1.+5.*fracb-fracnu-fracdr)/20./(1.-fracnu-fracdr)*om*tau)  * ppr->curvature_ini * s2_squared;
      ...
      if ((pba->has_ur == _TRUE_) || (pba->has_ncdm == _TRUE_) || (pba->has_dr == _TRUE_) || (pba->has_idr == _TRUE_)) {
        ...
        shear_ur = ktau_two/(45.+12.*fracnu+12.*fracdr) * (3.*s2_squared-1.) * (1.+(4.*fracnu+4.*fracdr-5.)/4./(2.*fracnu+2.*fracdr+15.)*tau*om) * ppr->curvature_ini;
        
        l3_ur = ktau_three*2./7./(12.*fracnu+12.*fracdr+45.)* ppr->curvature_ini;//TBC
      ...
      else { // For ppt->idr_nature == idr_free_streaming or no idr
      
      eta = ppr->curvature_ini * (1.-ktau_two/12./(15.+4.*(fracnu+fracdr))*(5.+4.*s2_squared*fracnu+4.*s2_squared*fracdr - (16.*fracnu*fracnu+32.*fracnu*fracdr+16.*fracdr*fracdr+280.*fracnu+280.*fracdr+325)/10./(2.*fracnu+2.*fracdr+15.)*tau*om));

        }

      //ppr->curvature_ini * (1.-ktau_two/12./(15.+4.*(fracnu+fracdr))*(5.+4.*s2_squared*fracnu+4.*s2_squared*fracdr - (16.*fracnu*fracnu+32.*fracnu*fracdr+16.*fracdr*fracdr+280.*fracnu+280.*fracdr+325)/10./(2.*fracnu+2.*fracdr+15.)*tau*om));//
    ...
    if ((ppt->has_cdi == _TRUE_) && (index_ic == ppt->index_ic_cdi)) {
      ...
      if ((pba->has_ur == _TRUE_) || (pba->has_ncdm == _TRUE_) || (pba->has_idr == _TRUE_)) {
    ...
    if ((ppt->has_nid == _TRUE_) && (index_ic == ppt->index_ic_nid)) {
      ...
      ppw->pv->y[ppw->pv->index_pt_delta_g] = ppr->entropy_ini*fracdr/(1.-fracdr)*(-1.+ktau_two/6.);
      ppw->pv->y[ppw->pv->index_pt_theta_g] = -ppr->entropy_ini*fracdr/(1.-fracdr)*k*k*tau*(1./4.-fracb/fracg*3./16.*om*tau);

      ppw->pv->y[ppw->pv->index_pt_delta_b] = ppr->entropy_ini*fracdr/(1.-fracdr)/8.*ktau_two;
      ppw->pv->y[ppw->pv->index_pt_theta_b] = ppw->pv->y[ppw->pv->index_pt_theta_g];

      if (pba->has_cdm == _TRUE_) {

        ppw->pv->y[ppw->pv->index_pt_delta_cdm] = -ppr->entropy_ini*fracdr*fracb/(1.-fracdr)/80.*ktau_two*om*tau;
    ...
    if ((ppt->has_drid == _TRUE_) && (index_ic == ppt->index_ic_drid)) {
      ...
      class_test((pba->has_idr == _FALSE_),
                 ppt->error_message,
                 "not consistent to ask for Dark radiation isocurvature (DRID) in absence of idr species!");
      ...
      shear_drad = ppr->entropy_ini*ktau_two*(4.*fracnu+15.-15.*fracdr)/30./(1.-fracdr)/(15.+4.*fracdr+4.*fracnu);
      
      }

      delta_ur = ppr->entropy_ini*fracdr/(1.-fracdr)*(-1.+ktau_two/6.);
      theta_ur = -ppr->entropy_ini*k*k*tau/4.*fracdr/(1.-fracdr);
    ...
    if (ppt->gauge == newtonian) {
      ...
      */
      ...
      if (pba->has_idr == _TRUE_) {
        ...
        delta_drad -= 4.*a_prime_over_a*alpha;
        theta_drad += k*k*alpha;
    ...
    if (pba->has_idr == _TRUE_){
      ...
      ppw->pv->y[ppw->pv->index_pt_delta_idr] = delta_drad;
      ppw->pv->y[ppw->pv->index_pt_theta_idr] = theta_drad;
      if (ppt->idr_nature == idr_free_streaming){
        if ((pba->has_idm_dr == _FALSE_)||((pba->has_idm_dr == _TRUE_)&&(ppw->approx[ppw->index_ap_tca_idm_dr] == (int)tca_idm_dr_off))){
      ...
      if (ppw->approx[ppw->index_ap_rsa_idr] == (int)rsa_idr_off) {
        ...
        if ((pba->has_idm_dr == _FALSE_)||((pba->has_idm_dr == _TRUE_)&&(ppw->approx[ppw->index_ap_tca_idm_dr] == (int)tca_idm_dr_off))) {
          ...
          tau_dot = pba->Geff;     
            ...
            dy[pv->index_pt_shear_idr] -= tau_dot*y[pv->index_pt_shear_idr];
            if (pba->has_idm_dr == _TRUE_)
            ...
            dy[pv->index_pt_l3_idr] -= tau_dot*y[pv->index_pt_l3_idr];
            if (pba->has_idm_dr == _TRUE_)
            ...
            for (l = 4; l < pv->l_max_idr; l++) {
              ...
              dy[pv->index_pt_delta_idr+l] -= tau_dot*y[pv->index_pt_delta_idr+l];
              if (pba->has_idm_dr == _TRUE_)
