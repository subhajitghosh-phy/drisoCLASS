...
if (input_verbose >= 1) {
      ...
      if (ppt->idr_nature == idr_free_streaming) {
       ...
       printf("idr nature --> free streaming\n");
       
       }
       
      if (ppt->idr_nature == idr_fluid) {
       
       printf("idr nature --> fluid\n");
       
       }
}  

class_call(parser_read_double(pfc,"log10_Geff",&param1,&flag1,errmsg),
             errmsg,
             errmsg);
   if (flag1 == _TRUE_) {
       if (ppt->idr_nature == idr_free_streaming){
       ...
       class_test( ppt->idr_nature != idr_free_streaming,
               errmsg,
               "The dark radiation (DR) nature is NOT FREE-STREAMING. log10_Geff does not have any effect on DR.",param1);
   }
   
class_call(parser_read_string(pfc,"dr_coupling",&string1,&flag1,errmsg),
               errmsg,
               errmsg);

    if (flag1 == _TRUE_) {

      if ((strstr(string1,"decouping") != NULL) || (strstr(string1,"decoup") != NULL)) {
        pba->dr_coupling = decoup;
      }

      else if ((strstr(string1,"recoupling") != NULL) || (strstr(string1,"recoup") != NULL)) {
        pba->dr_coupling = recoup;
      }

      else {
        class_stop(errmsg,"incomprehensible input '%s' for the field 'dr_coupling'. Options: decoupling(decoup) or recoupling(recoup)",string1);
      }
    if (input_verbose >= 1) {
      
      if (pba->dr_coupling == decoup) {
       
       printf("DR coupling --> Decoupling\n");
       
       }
       
      if (pba->dr_coupling == recoup) {
       
       printf("DR coupling --> Recoupling\n");
  ...
  if ((flag1 == _TRUE_) && (N_ncdm > 0)){
	...
	for(n=0; n<N_ncdm; n++){
		pba->T_ncdm[n] = pba->T_ncdm_all;
	}
    if (input_verbose > 1) {
		fprintf(stdout," All N_{ncdm} = %d species set to common T_ncdm = %f.\n",pba->N_ncdm,pba->T_ncdm_all);
	}
     }
    else{
  ...
  if (ppm->primordial_spec_type == two_scales) {
    ...
    if (ppt->has_scalars == _TRUE_) {
     ...
     class_test(prr2<=0.,errmsg,"enter strictly positive scale P_{RR}^2");
      ...
      if ((ppt->has_bi == _TRUE_) ||
       ...
       class_call(parser_read_double(pfc,"log10_P_{II}^1",&param3,&flag3,errmsg),errmsg,errmsg);
       class_test(class_at_least_two_of_three(flag1,flag2,flag3),
             errmsg,
             "In input file, you can only enter one of P_{II}^1,phP_{II}^1,log10_P_{II}^1, choose one");
       if (flag1 == _TRUE_) {
          pii1 = param1;
          if (input_verbose > 1) printf("You passed pii1 = %e \n" , param1);
          }
       if (flag3 == _TRUE_) {
          pii1 = pow(10.0,param3);
          if (input_verbose > 1) printf("You passed log10_pii1 = %e i.e, pii1 = %e \n" , param3, pii1);
          }
       if (flag2 == _TRUE_) {
       ...
       pri1 = pow(10.0, log10_pri1);
        ...
        //--------------------------------------pii2------------------------------------------//
       class_call(parser_read_double(pfc,"P_{II}^2",&param1,&flag1,errmsg),errmsg,errmsg);
       class_call(parser_read_double(pfc,"phP_{II}^2",&param2,&flag2,errmsg),errmsg,errmsg);
       class_call(parser_read_double(pfc,"log10_P_{II}^2",&param3,&flag3,errmsg),errmsg,errmsg);
       class_test(class_at_least_two_of_three(flag1,flag2,flag3),
             errmsg,
             "In input file, you can only enter one of P_{II}^2,phP_{II}^2,log10_P_{II}^2, choose one");
       if (flag1 == _TRUE_) {
          pii2 = param1;
          if (input_verbose > 1) printf("You passed pii2 = %e \n" , param1);
          }
       if (flag3 == _TRUE_) {
          pii2 = pow(10.0,param3);
          if (input_verbose > 1) printf("You passed log10_pii2 = %e i.e, pii2 = %e \n" , param3, pow(10.0,param3));
          }
       if (flag2 == _TRUE_) {
          pii2 = param2/pow((pba->Omega0_idr/7.*8./pow(4./11.,4./3.)/pba->Omega0_g),2);
          if (input_verbose > 1) printf("You passed (N_idr)^2*pii2 = %e i.e, pii2 = %e with N_idr = %e \n" , param2, param2/pow((pba->Omega0_idr/7.*8./pow(4./11.,4./3.)/pba->Omega0_g),2), (pba->Omega0_idr/7.*8./pow(4./11.,4./3.)/pba->Omega0_g) );
          }
       //--------------------------------------pri1------------------------------------------//
       class_call(parser_read_double(pfc,"P_{RI}^1",&param1,&flag1,errmsg),errmsg,errmsg);
       class_call(parser_read_double(pfc,"phP_{RI}^1",&param2,&flag2,errmsg),errmsg,errmsg);
       class_call(parser_read_double(pfc,"log10_P_{RI}^1",&param3,&flag3,errmsg),errmsg,errmsg);
       class_test(class_at_least_two_of_three(flag1,flag2,flag3),
             errmsg,
             "In input file, you can only enter one of P_{RI}^1,phP_{RI}^1,log10_P_{RI}^1, choose one");
       if (flag1 == _TRUE_) {
          pri1 = param1;
          if (input_verbose > 1) printf("You passed pri1 = %e \n" , param1);
          }
       if (flag3 == _TRUE_) {
          pri1 = pow(10.0,param3);
          if (input_verbose > 1) printf("You passed log10_pri1 = %e i.e, pri1 = %e \n" , param3, pri1);
          }
       if (flag2 == _TRUE_) {
          pri1 = param2/pow((pba->Omega0_idr/7.*8./pow(4./11.,4./3.)/pba->Omega0_g),1);
          if (input_verbose > 1) printf("You passed (N_idr)*pri1 = %e i.e, pri1 = %e with N_idr = %e \n" , param2, param2/pow((pba->Omega0_idr/7.*8./pow(4./11.,4./3.)/pba->Omega0_g),1), (pba->Omega0_idr/7.*8./pow(4./11.,4./3.)/pba->Omega0_g) );
          }
    ...
    if (ppt->has_scalars == _TRUE_) {
      ...
      if (ppt->has_drid == _TRUE_) {
        ...
        class_read_double("n_drid",ppm->n_drid);
      ...
      if ((ppt->has_ad == _TRUE_) && (ppt->has_drid == _TRUE_)) {
        ...
        class_read_double_one_of_two("n_ad_drid","n_drid_ad",ppm->n_ad_drid);
